...
def main():
    ...
    try:
        ...
        while True:
            ...
            if mission_process == 0 and not t_hover.is_alive() and not t_qr.is_alive(): # rotate counter-clockwise until find the marker
                ...
                if iter_1 == 25: # 5 iter -> 0.1s
                    ...
                    throttle_pid = 1.3*(PD(e_y, e_y_old, del_t,K_P,K_D))
                    ...
                    # if error is stable for 0.5s -> go to next process 
                ...
                if iter_2 == 10:
                    ...
                    throttle_pid = 1.3*(PD(e_y, e_y_old, del_t, K_P, K_D))
                    pitch_pid = -1.7*(PD(e_dist, e_dist_old, del_t, K_P, K_D))
            ...
            ## after mission process 4 -> mission_state + 1 and mission process = 0
                ...
                if not t_qr.is_alive() and altitude < 50:
                    t_up = Thread(target=up, args=(20,))
                    t_up.start()

            if mission_state == 3 and not t_qr.is_alive()  :
                drone.land()
                drone.end()  
